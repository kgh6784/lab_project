...
if __name__ == '__main__':
    with initialize('configs'):
        cfg = compose('benchmark')

    # models = [create_model('resnet18', num_classes=10), ResNetSimCLR(out_dim=10), BYOLResNet18(),
    #           DualAttentionNet(num_classes=10)]
    models = [DualAttentionNet(num_classes=10), BYOLResNet18(), ResNetSimCLR(out_dim=10),
              create_model('resnet18', num_classes=10)]

    for model in models:
        results = benchmark_model(cfg, model)
